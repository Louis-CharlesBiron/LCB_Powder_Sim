...
function createPhysicsCore(CONFIG, M, G, S, SG, SP, D) {
    ...
    function physicsStep(
        ...
        for (;countIndex<count;countIndex++) {
            ...
            if (mat === SAND) {
                ...
                let dx = 0, dy = 0, velX = indexVelX[i]
                ...
                if (indexFlags[i]&COLLISION_BOTTOM) {
                    ...
                    // SKIP IF CONTAINED
                    if (mat & DOWN_MAIN_CONTAINED_SKIPABLE && (m_B^mat) === 0 && ((m_BR^mat|m_BL^mat) === 0 || (m_R^mat|m_L^mat) === 0) && abs(velX) <= X_VELOCITY_SKIP_THRESHOLD) {skip1++;continue}pass1++
                ...
                if (!(indexFlags[i]&COLLISION_BOTTOM)) {
                    ...
                    velX = decelarate(indexVelX, i, GROUND_VELX_DECELERATION_RATE*deltaTime)
                }
                ...
                if (!hasPhysicsMovements) {skip2++;continue}pass2++
                ...
                if (absGdy > 1) {
                    ...
                    if (hasCollision) {
                        ...
                        //const vel_Dest = indexVelY[gridIndexes[gi_Dest]], velDiff = abs(vel_Dest-indexVelY[i])
                        //console.log(i, [newX, newY], "DIFF:", velDiff, "VEL:", +indexVelY[i]?.toFixed(1), "->", +indexVelY[gridIndexes[gi_Dest]]?.toFixed(1), "|", (vel_Dest === undefined || m_Dest & STATIC || gi_Dest === gi), vel_Dest < indexVelY[i], velDiff > Y_COLLISION_VELOCITY_DIFFERENCE_THRESHOLD)
                        //if ((vel_Dest === undefined || m_Dest & STATIC || gi_Dest === gi) || (vel_Dest < indexVelY[i] && velDiff > Y_COLLISION_VELOCITY_DIFFERENCE_THRESHOLD)) {
                        //    console.log("RESET VEL", i, indexVelY[i], "->", indexVelY[gridIndexes[gi_Dest]], "|", velDiff)
                        //    indexVelY[i] = indexVelY[i]/2
                        //    indexFlags[i] |= dirGdy===1 ? COLLISION_BOTTOM : COLLISION_TOP
                        //}
            ...
            if (mat === WATER) {
                ...
                if (indexFlags[i]&COLLISION_BOTTOM) {
                    ...
                    // SKIP IF CONTAINED
                    // hasColX && 
                    if (mat & DOWN_MAIN_CONTAINED_SKIPABLE && (m_B^mat|m_BR^mat|m_BL^mat|m_R^mat|m_L^mat) === 0 && abs(velX) <= X_VELOCITY_SKIP_THRESHOLD) {skip1++;continue}pass1++
        ...
        if (CONFIG.showSkips) {
            ...
            console.log("1:", skip1, "2:", skip2, "3:", skip3, "|", ((skipped/total)*100).toFixed(1)+"%", "(active: "+(total-skip1)+")", "p1:", pass1, "p2:", pass2, "p3:", pass3)
            skip1 = skip2 = skip3 = 0
            pass1 = pass2 = pass3 = 0
        }
    ...
    function decelarate(indexVel, i, rate) {
        const vel = indexVel[i]
        if (vel > 0) return indexVel[i] = vel > rate ? vel-rate : 0
        else return indexVel[i] = vel < -rate ? vel+rate : 0
    }
